...
let decode = (~blurhash: string, ~width: int, ~height: int, ~punch: int): pixels => {
  let sizeFlag = blurhash->Js.String2.charAt(0)->BlurHash_Base83.decode
  let numY = (sizeFlag->Int.toFloat /. 9.)->Js.Math.floor + 1
  let numX = mod(sizeFlag, 9) + 1

  let quantisedMaximumValue = blurhash->Js.String2.charAt(1)->BlurHash_Base83.decode
  let maximumValue = (quantisedMaximumValue + 1)->Int.toFloat /. 166.

  let colors =
    Array.range(1, numX * numY - 1)->Array.reduce(
      [blurhash->Js.String2.substring(~from=2, ~to_=6)->BlurHash_Base83.decode->decodeDC],
      (colors, i) => {
        let color =
          blurhash
